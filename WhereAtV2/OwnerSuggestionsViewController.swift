//
//  OwnerSuggestionsViewController.swift
//  WhereAtV2
//
...
class OwnerSuggestionsViewController: UIViewController {

    var detailMeetup: Meetup!
  
    //there will be a meetup with the final location included.
    var names = [String]()
    ...
    @IBAction func switched(_ sender: UISwitch) {
        if (sender.tag == 9) {//means first buton
    ...
    func send_suggestions() {
        ...
        let json: [String: Any] = ["event_id": detailMeetup._id, "owner_picks": userPicks]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if (segue.identifier == "unwindOwnerPicksToFeed") {
            send_suggestions()
...
//            let secondViewController = navController.topViewController as! SecondViewController
    ...
    override func shouldPerformSegue(withIdentifier identifier: String, sender: Any?) -> Bool {
        if (identifier == "unwindOwnerPicksToFeed") {
            if (switch1.isOn) {
    ...
    func google_suggestions(isTest:Bool, withCompletionHandler:@escaping (_ something:[[String:Any]]) -> Void) {
       //hardcoded
//        let lat = "42.3004620080885"
//        let long = "-83.7018211725112"
        
        let lat = detailMeetup.equidist_lat
        let long = detailMeetup.equidist_long
